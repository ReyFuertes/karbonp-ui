...
import { ExpensesMock } from 'src/app/shared/mock/getting-started.mock';
import { formatToJson } from 'src/app/shared/util/formatting';
import { AppState } from 'src/app/store';

@Component({
  selector: 'kp-expenses-get-started',
  templateUrl: './expenses-get-started.component.html'
})
export class ExpensesGetStartedComponent extends GenericDestroy {
  public application: IApplication;
  public mock: IGettingStartedMock;

  constructor(private store: Store<AppState>) {
    super();
    this.mock = ExpensesMock;
    this.store.pipe(select(getEmployeeAppByNameSelector(AppDashboardType.Expenses)))
